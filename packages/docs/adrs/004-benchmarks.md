...
Comparing Classic vs Advanced requires reproducible conditions despite LLM stochastic behavior. The benchmark must measure cost, quality, and efficiency dimensions while handling asymmetric metrics (drift data is Advanced-only).
...
5. Classic drift metrics reported as `null` with reason annotation, not as zero (honest absence > fake data)
